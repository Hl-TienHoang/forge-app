...
class LabelsExtension extends Autodesk.Viewing.Extension {
    ...
    onClickAddLabel(e) {
        ...
        this.addLabel(dbid, this.computeCenterPoint(dbid));
    ...
    computeCenterPoint(dbid) {
        ...
        frags.getWorldBounds(dbid, bbox);
        return bbox.center();

    }

    addLabel(title, cp) {
        fetch('/api/forge/oss/info/' + title)
            .then(res => {
                return res.json()
            })
            .then(data => {
                ...
                div.className = 'label';
                div.textContent = title + ': ' + data.value;
